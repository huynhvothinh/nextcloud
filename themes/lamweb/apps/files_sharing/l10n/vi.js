{
    "translations": {
        "Shared with you": "Chia sẻ với bạn",
        "Shared with others": "Chia sẻ với người khác",
        "Shared by link": "Chia sẻ theo liên kết",
        "Nothing shared with you yet": "Bạn chưa được chia sẻ gì cả",
        "Files and folders others share with you will show up here": "Các tệp và thư mục mà những người khác chia sẻ với bạn sẽ hiển thị ở đây ",
        "Nothing shared yet": "Chưa có gì được chia sẻ",
        "Files and folders you share will show up here": "Tập tin và thư mục bạn chia sẻ sẽ được hiển thị tại đây.",
        "No shared links": "Chưa có liên kết chia sẻ",
        "Files and folders you share by link will show up here": "Các tệp và thư mục bạn chia sẻ theo liên kết sẽ hiển thị tại đây ",
        "You can upload into this folder": "Bạn có thể tải lên thư mục này ",
        "No compatible server found at {remote}": "Không tìm thấy máy chủ tương thích tại {remote} ",
        "Invalid server URL": "URL máy chủ không hợp lệ ",
        "Failed to add the public link to your Nextcloud": "Không thể thêm liên kết công khai vào Nextcloud ",
        "Share": "Chia sẻ ",
        "No expiration date set": "No expiration date set",
        "Shared by": "Chia sẻ bởi",
        "Sharing": "Chia sẻ",
        "File shares": "Chia sẻ tệp ",
        "Downloaded via public link": "Đã tải xuống qua liên kết công khai ",
        "Downloaded by {email}": "Được tải xuống bởi {email} ",
        "{file} downloaded via public link": "{file} được tải xuống qua liên kết công khai ",
        "{email} downloaded {file}": "{email} đã tải xuống {file} ",
        "Shared with group {group}": "Được chia sẻ với nhóm {group} ",
        "Removed share for group {group}": "Đã xóa phần chia sẻ cho nhóm {group} ",
        "{actor} shared with group {group}": "{actor} được chia sẻ với nhóm {group} ",
        "{actor} removed share for group {group}": "{actor} đã xóa chia sẻ cho nhóm {group} ",
        "You shared {file} with group {group}": "Bạn đã chia sẻ {file} với nhóm {group} ",
        "You removed group {group} from {file}": "Bạn đã xóa nhóm {group} khỏi {file} ",
        "{actor} shared {file} with group {group}": "{actor} đã chia sẻ {file} với nhóm {group} ",
        "{actor} removed group {group} from {file}": "{actor} đã xóa nhóm {group} khỏi {file} ",
        "Shared as public link": "Được chia sẻ dưới dạng liên kết công khai ",
        "Removed public link": "Đã xóa liên kết công khai ",
        "Public link expired": "Liên kết công khai đã hết hạn ",
        "{actor} shared as public link": "{actor} được chia sẻ dưới dạng liên kết công khai ",
        "{actor} removed public link": "{actor} đã xóa liên kết công khai ",
        "Public link of {actor} expired": "Liên kết công khai của {actor} đã hết hạn ",
        "You shared {file} as public link": "Bạn đã chia sẻ {file} dưới dạng liên kết công khai ",
        "You removed public link for {file}": "Bạn đã xóa liên kết công khai cho {file} ",
        "Public link expired for {file}": "Liên kết công khai đã hết hạn cho {file} ",
        "{actor} shared {file} as public link": "{actor} đã chia sẻ {file} dưới dạng liên kết công khai ",
        "{actor} removed public link for {file}": "{actor} đã xóa liên kết công khai cho {file} ",
        "Public link of {actor} for {file} expired": "Liên kết công khai của {actor} cho {file} đã hết hạn ",
        "{user} accepted the remote share": "{user} đã chấp nhận chia sẻ từ xa ",
        "{user} declined the remote share": "{user} đã từ chối chia sẻ từ xa ",
        "You received a new remote share {file} from {user}": "Bạn đã nhận được một phần từ xa mới {file} từ {user} ",
        "{user} accepted the remote share of {file}": "{user} đã chấp nhận chia sẻ từ xa của {file} ",
        "{user} declined the remote share of {file}": "{user} đã từ chối chia sẻ từ xa của {file} ",
        "{user} unshared {file} from you": "{user} đã thôi chia sẻ {file} từ bạn ",
        "Shared with {user}": "Được chia sẻ với {user} ",
        "Removed share for {user}": "Đã xóa chia sẻ cho {user} ",
        "{actor} shared with {user}": "{actor} được chia sẻ với {user} ",
        "{actor} removed share for {user}": "{actor} đã xóa chia sẻ cho {user} ",
        "Shared by {actor}": "Được chia sẻ bởi {actor} ",
        "{actor} removed share": "{actor} đã xóa chia sẻ ",
        "You shared {file} with {user}": "Bạn đã chia sẻ {file} với {user} ",
        "You removed {user} from {file}": "Bạn đã xóa {user} khỏi {file} ",
        "{actor} shared {file} with {user}": "{actor} đã chia sẻ {file} với {user} ",
        "{actor} removed {user} from {file}": "{actor} đã xóa {user} khỏi {file} ",
        "{actor} shared {file} with you": "{actor} đã chia sẻ {file} với bạn ",
        "{actor} removed you from {file}": "{actor} đã xóa bạn khỏi {file} ",
        "A file or folder shared by mail or by public link was <strong>downloaded</strong>": "Tệp hoặc thư mục được chia sẻ qua thư hoặc bằng liên kết công khai đã được <strong> tải xuống </ strong> ",
        "A file or folder was shared from <strong>another server</strong>": "Tệp hoặc thư mục được chia sẻ từ <strong> máy chủ khác </ strong> ",
        "A file or folder has been <strong>shared</strong>": "Tệp hoặc thư mục đã được <strong> chia sẻ </ strong> ",
        "Wrong share ID, share doesn't exist": "ID chia sẻ sai, chia sẻ không tồn tại ",
        "could not delete share": "không thể xóa chia sẻ ",
        "Could not delete share": "Không thể xóa chia sẻ ",
        "Please specify a file or folder path": "Vui lòng chỉ định đường dẫn tệp hoặc thư mục ",
        "Wrong path, file/folder doesn't exist": "Đường dẫn sai, tệp / thư mục không tồn tại ",
        "Could not create share": "Không thể tạo chia sẻ ",
        "invalid permissions": "các quyền không hợp lệ ",
        "Please specify a valid user": "Vui lòng chỉ định người dùng hợp lệ ",
        "Group sharing is disabled by the administrator": "Tính năng chia sẻ nhóm bị tắt bởi quản trị viên ",
        "Please specify a valid group": "Vui lòng chỉ định nhóm hợp lệ ",
        "Public link sharing is disabled by the administrator": "Chia sẻ liên kết công khai bị vô hiệu hóa bởi quản trị viên ",
        "Public upload disabled by the administrator": "Tải lên công khai bị vô hiệu hóa bởi quản trị viên ",
        "Public upload is only possible for publicly shared folders": "Tải lên công khai chỉ có thể cho các thư mục được chia sẻ công khai ",
        "Invalid date, date format must be YYYY-MM-DD": "Ngày, định dạng ngày không hợp lệ phải là YYYY-MM-DD ",
        "Sharing %s failed because the back end does not allow shares from type %s": "Chia sẻ% s không thành công vì phần cuối không cho phép chia sẻ từ loại% s ",
        "You cannot share to a Circle if the app is not enabled": "Bạn không thể chia sẻ với Vòng kết nối nếu ứng dụng không được bật ",
        "Please specify a valid circle": "Vui lòng chỉ định vòng kết nối hợp lệ ",
        "Unknown share type": "Loại chia sẻ không xác định ",
        "Not a directory": "Không phải thư mục ",
        "Could not lock path": "Không thể khóa đường dẫn ",
        "Wrong or no update parameter given": "Thông số cập nhật sai hoặc không có ",
        "Can't change permissions for public share links": "Không thể thay đổi quyền đối với liên kết chia sẻ công khai ",
        "Cannot increase permissions": "Không thể tăng quyền ",
        "shared by %s": "được chia sẻ bởi% s ",
        "Download": "Tải về",
        "Direct link": "Link trực tiếp",
        "Add to your Nextcloud": "Thêm vào Nextcloud của bạn ",
        "Share API is disabled": "API chia sẻ bị tắt ",
        "File sharing": "Chia sẻ tệp ",
        "This share is password-protected": "Chia sẻ này được bảo vệ bằng mật khẩu ",
        "The password is wrong. Try again.": "Mật khẩu sai. Xin hãy thử lại.",
        "Password": "Mật khẩu",
        "No entries found in this folder": "Chưa có mục nào trong thư mục",
        "Name": "Tên",
        "Share time": "Chia sẻ thời gian",
        "Expiration date": "Ngày hết hạn ",
        "Sorry, this link doesn’t seem to work anymore.": "Rất tiếc, liên kết này dường như không hoạt động nữa. ",
        "Reasons might be:": "Lý do có thể là:",
        "the item was removed": "đối tượng đã bị xóa",
        "the link expired": "liên kết đã hết hạn",
        "sharing is disabled": "việc chia sẻ bị cấm",
        "For more info, please ask the person who sent this link.": "Để biết thêm thông tin, vui lòng hỏi người đã gửi liên kết này. ",
        "Download %s": "Tải về %s",
        "Upload files to %s": "Tải tệp lên% s ",
        "Select or drop files": "Chọn hoặc thả tệp ",
        "Uploading files…": "Đang tải tệp lên… ",
        "Uploaded files:": "Tệp đã tải lên: ",
        "%s is publicly shared": "% s được chia sẻ công khai "
    },
    "pluralForm": "nplurals=1; plural=0;"
}